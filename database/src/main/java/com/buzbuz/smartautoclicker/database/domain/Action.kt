...
sealed class Action {
    ...
    data class Input(
        ...
        override fun toEntity(): ActionEntity {
            if (!isComplete()) throw IllegalStateException("Can't transform to entity, Input is incomplete.")
        ...
        override fun deepCopy(): Input = copy(name = "" + name)
    }
    /**
     * Pause action.
     *
    ...
    data class ButtonPress(
        ...
        var eventId: Long,
        var name: String? = null,
        var button: ButtonPressType? = null,
    ) : Action() {



        override fun isComplete(): Boolean =
            name != null && button != null
        ...
        override fun deepCopy(): ButtonPress = copy(name = "" + name)
    }
...
internal fun ActionEntity.toAction(): Action {
    return when (type) {
        ...
        ActionType.INPUT -> Action.Input(id, eventId, name, text!!)
        ActionType.BUTTON_PRESS -> Action.ButtonPress(id, eventId, name, button!!)
