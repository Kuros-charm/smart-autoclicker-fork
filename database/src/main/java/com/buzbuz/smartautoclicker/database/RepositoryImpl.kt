...
internal class RepositoryImpl internal constructor(
    ...
    override suspend fun getBitmap(id: String,byteArray: ByteArray, width: Int, height: Int) = bitmapManager.loadBitmap(id, byteArray, width, height)
    ...
    private suspend fun saveNewConditionsBitmap(conditions: List<Condition>) {
        conditions.forEach { condition ->
            if (condition.path == null) {
                ...
                condition.bitmapByteArray = bitmapManager.bitmapToByteArray(condition.bitmap)
                condition.path =  "${condition.bitmapByteArray.hashCode()}"
...
//    }
    ...
    override suspend fun doUpgrade() {
        this.getAllEvents().collect {
            it.map {
                if (it.conditions!!.filter { it.bitmapByteArray == null }.size > 0){
                    conditionsUpdater.refreshUpdateValues(
                        it.conditions!!.map { it.toEntity() },
                        it.conditions!!.map {
                            it.copy(bitmapByteArray = bitmapManager.bitmapToByteArray(bitmapManager.loadBitmap2(it.path!!,it.area.width(), it.area.height())!!)).toEntity()

                        }
                    )
                    actionsUpdater.refreshUpdateValues(it.actions!!.map { it.toEntity() }, it.actions!!.map { it.toEntity() })

                    eventDao.updateEvent(it.toEntity(), actionsUpdater, conditionsUpdater)
                }
