...
class ActionConfigDialog(
    ...
    private val title: Pair<Int, Int> = when (action) {
        ...
        is Action.Input -> R.string.dialog_action_type_input to R.drawable.ic_rename
        is Action.ButtonPress -> R.string.dialog_action_type_button_press to R.drawable.ic_hint_resize_up
                ...
                launch {
                    viewModel?.actionValues?.collect { actionValues ->
                        when (actionValues) {
                            ...
                            is ActionConfigModel.InputActionValues -> setupInputUi(actionValues)
                            is ActionConfigModel.ButtonActionValues -> setupButtonUi(actionValues)
                        }
                    }
                }
    ...
    private fun setupInputUi(inputValues: ActionConfigModel.InputActionValues) {
        viewBinding.apply {
            includeInputConfig.apply {
                actionConfigLayoutInput.visibility = View.VISIBLE

                editInputText.apply {
                    setSelectAllOnFocus(true)
                    addTextChangedListener(object : OnAfterTextChangedListener() {
                        override fun afterTextChanged(s: Editable?) {
                            (viewModel?.actionValues?.value as ActionConfigModel.InputActionValues)
                                .setText(if (!s.isNullOrEmpty()) s.toString() else null)
                        }
                    })
                }
            }
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                launch {
                    inputValues.text.collect { txt ->
                        viewBinding.includeInputConfig.editInputText.apply {
    ...
    private fun setupButtonUi(buttonValues: ActionConfigModel.ButtonActionValues) {


        viewBinding.apply {
            includeButtonConfig.apply {
                actionConfigLayoutButton.visibility = View.VISIBLE

                editInputButton.apply {
                    adapter = ArrayAdapter(this@ActionConfigDialog.context, android.R.layout.simple_list_item_1,ButtonPressType.values())
                    onItemSelectedListener = (object : AdapterView.OnItemSelectedListener {
                        override fun onItemSelected(p0: AdapterView<*>?, p1: View?, pos: Int, p3: Long) {
                            val item = editInputButton.selectedItem as ButtonPressType
                            (viewModel?.actionValues?.value as ActionConfigModel.ButtonActionValues)
                                .setButton(item)
                        }
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                launch {
                    buttonValues.button.collect { btn ->
                        viewBinding.includeButtonConfig.editInputButton.apply {
                            setSelection( (adapter as ArrayAdapter<ButtonPressType>).getPosition(btn) )
                        }
                    }
                }
            }
