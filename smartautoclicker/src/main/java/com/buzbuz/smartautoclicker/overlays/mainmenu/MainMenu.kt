...
class MainMenu(context: Context, private val scenario: Scenario) : OverlayMenuController(context) {
    ...
    override fun onMenuItemLongClicked(viewId: Int): Boolean? {
        when (viewId) {
            R.id.btn_click_list ->  {
                var historyString = viewModel?.getLastestExecutedEvents()?.takeLast(10)?.reversed()?.joinToString("\n")

                if (historyString == null || historyString == ""){
                    historyString = "No execution history"
                }
                Toast.makeText(context, historyString, Toast.LENGTH_SHORT).show()
                return true
            }
        }
        return false
