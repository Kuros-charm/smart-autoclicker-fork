    ...
    override fun onDialogCreated(dialog: AlertDialog) {
        viewBinding.apply {
            ...
            editMinDelay.apply {
                ...
                addTextChangedListener(object : OnAfterTextChangedListener() {
                    override fun afterTextChanged(s: Editable?) {
                        val minDelay = viewBinding.editMinDelay.text.toString()
                        viewModel?.setMinDelayBetweenAction(if (minDelay.isNotEmpty()) minDelay.toInt() else null)
                    }
                })
            }
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                ...
                launch {
                    viewModel?.minDelay?.collect { minDelay ->
                        viewBinding.editMinDelay.apply {
                            val minDelayText = minDelay?.toString()
                            setText(minDelayText)
                            setSelection(minDelayText?.length ?: 0)
                        }
                    }
                }
            }
        ...
        when (overlayType) {
            ...
            is SubOverlay.ActionTypeSelection -> {
                showSubOverlay(MultiChoiceDialog(
                    ...
                    choices = listOf(ActionTypeChoice.Click, ActionTypeChoice.Swipe, ActionTypeChoice.Pause, ActionTypeChoice.Input,ActionTypeChoice.Button),
                    onChoiceSelected = { choiceClicked ->
                        viewModel?.let { model ->
