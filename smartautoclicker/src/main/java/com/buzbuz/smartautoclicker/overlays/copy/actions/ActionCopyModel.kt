...
class ActionCopyModel(context: Context) : OverlayViewModel(context) {
    ...
    fun getNewActionForCopy(action: Action): Action =
        when (action) {
            ...
            is Action.Input -> action.copy(id = 0, name = "" + action.getActionName())
            is Action.ButtonPress -> action.copy(id = 0, name = "" + action.getActionName())
    ...
    private fun Action.toActionItem(): ActionCopyItem.ActionItem {
        val item = when (this) {
            ...
            is Action.Input -> ActionCopyItem.ActionItem(
            ...
            is Action.ButtonPress -> ActionCopyItem.ActionItem(
                icon = R.drawable.ic_hint_resize_up,
                name = name!!,
                details = context.getString(
                    R.string.dialog_action_copy_pause_details,
                   button?.displayName
