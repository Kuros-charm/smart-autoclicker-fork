...
class SmartAutoClickerService : AccessibilityService() {
    ...
    inner class LocalService {
        ...
        fun start(resultCode: Int, data: Intent, scenario: Scenario) {
            ...
            detectorEngine = DetectorEngine.getDetectorEngine(this@SmartAutoClickerService).apply {
                ...
                setOnTextActionListener { actionId, bundle ->
                    var node = findFocusedEditText(rootInActiveWindow);
                    if (node == null){
                        var wins = getWindows()
                        for (i in 0..wins.size-1){
                            val node = findFocusedEditText(wins[i].root);
                            if (node != null){
                                break
                            }
                        }
                    }
                    node?.performAction(actionId, bundle);
        ...
        fun findFocusedEditText(node: AccessibilityNodeInfo?): AccessibilityNodeInfo?{
            ...
            if ((node.className?.indexOf("EditText") != -1) and node.isFocused()){
                return node
            }
            for (i in 0..node.childCount-1){
                val childResult = findFocusedEditText(node.getChild(i))
                if (childResult != null){
                    return childResult
                }
            }
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        //LOCAL_SERVICE_INSTANCE?.node = event?.getSource();
    }
}
