...
internal class ConditionDetector(private val cache: Cache) {
    ...
    private fun checkCondition(condition: Condition) : Boolean {
        ...
        cache.pixelsCache.get(condition)?.let { pixels ->
            ...
            // Count pixel within margin
            cache.currentDiff = 0
            for (i in pixels.second.indices) {
                if (
                    (abs((pixels.first[i] shr 16 and 0xff) - (pixels.second[i] shr 16 and 0xff)) < margin) and
                    (abs((pixels.first[i] shr 8 and 0xff) - (pixels.second[i] shr 8 and 0xff)) < margin ) and
                    (abs((pixels.first[i] and 0xff) - (pixels.second[i] and 0xff)) < margin)
                ){
                    cache.currentDiff +=0
                }else {
                    cache.currentDiff += 3L * 255
                }
            }
